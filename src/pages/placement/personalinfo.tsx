...
import styles from "@/styles/placement/PersonalInfo.module.css";
...
const PersonalInfo: React.FC = () => {
  const [formData, setFormData] = useState({
    ...
    phoneNumber: "",
    department: "",
    role: "Placement Officer", // Default role
    ...
    const fetchProfile = async () => {
      try {
        ...
        setFormData({
          ...
          role: data.role || "Placement Officer",
  ...
  const handleSave = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.message || "Failed to update placement officer information.");
      }
      ...
      console.error("Error updating placement officer info:", error);
    }
  };

  return (
    <div className={styles.container}>
      <h2 className={styles.title}>Placement Officer Profile</h2>
      ...
      <div className={styles.formGrid}>
        ...
        <div className={styles.inputGroup}>
          ...
          {errors.department && <p className={styles.errorText}><FaExclamationCircle /> {errors.department}</p>}
        </div>
      </div>
